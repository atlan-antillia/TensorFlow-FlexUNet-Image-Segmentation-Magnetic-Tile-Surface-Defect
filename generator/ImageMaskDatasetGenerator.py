...
class ImageMaskDatasetGenerator:
  def __init__(self, size=512, augmentation=True):
    #      rgb_map = {Blowhole:red, Break:green, Crack:blue, Fray:yellow,  Uneven:mazenda}
    self.RGB_COLOS = [(255, 0, 0), (0,255,0), (0,0,255),(255,255,0),(255,0,255),]
  ...
  def generate(self, root_dir, output_images_dir, output_masks_dir):
     ...
     for sub_dir in sub_dirs:
       ...
       full_subdir = os.path.join(root_dir, sub_dir + "/Imgs")
       if not os.path.exists(full_subdir):
         print("Skipping", full_subdir )
         continue       
       print(n, sub_dir)
       (r, g, b) = self.RGB_COLOS[n]
       bgr_color = (b, g, r)
       print(bgr_color)
   
       n += 1
       image_files = sorted(glob.glob(full_subdir + "/*.jpg"))
       mask_files  = sorted(glob.glob(full_subdir + "/*.png"))
       num_images  = len(image_files)
       num_masks   = len(mask_files)
       num = min(num_images, num_masks)
       fname = sub_dir.split("_")[1]
       for i in range(num):
          self.generate_image_file(image_files[i], fname, i, output_images_dir) 
          self.generate_mask_file(mask_files[i], fname, bgr_color, i, output_masks_dir) 
  ...
  def generate_image_file(self, image_file, fname, i, output_dir):
      image = cv2.imread(image_file)
      filename = fname + "_" + str(i) + ".jpg"
      output_filepath = os.path.join(output_dir, filename)
      cv2.imwrite(output_filepath, image)
      print("Saved {}".format(output_filepath))
      if self.augmentation:
        self.augment(image, filename, output_dir, border=(0, 0, 0),     mask=False)
        
  def generate_mask_file(self, image_file, fname, bgr_color, i, output_dir):
      image = cv2.imread(image_file)
      image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
      image = self.colorize_mask(image,  bgr_color)
      filename = fname + "_" + str(i) + ".png"
      output_filepath = os.path.join(output_dir, filename)
      cv2.imwrite(output_filepath, image)
      print("Saved {}".format(output_filepath))
      if self.augmentation:
        self.augment(image, filename, output_dir, border=(0, 0, 0),     mask=True)
...
if __name__ == "__main__":
  try:
     root_dir   = "./Magnetic-tile/"
     output_dir = "./Augmented-Magnetic-Tile-master"
     ...
     generator.generate(root_dir, output_images_dir, output_masks_dir)
